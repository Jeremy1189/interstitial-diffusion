function [fitness_value,tracks_cell, type_set_cell] = fitness_D(pop,kMC_init_para)

 tic;
 avg_barrier =pop;
 [R, ~] = size(pop);
 D_set=zeros(R,1);
 tracks_cell=cell(R,1);
 type_set_cell=cell(R,1);
...
%     toc;
    ...
    type_set(count,:) = interstitial_per_types([interstitial_ID_back,interstitial_ID_mig,target_interstitial_ID]);
...
tracks_cell{i}=tracks;
type_set_cell{i} = type_set;
end
